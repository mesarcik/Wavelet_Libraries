function [recovered] = DWT2_2(X,XN,T,V,plot_bool,wname)
	Lo_D	=	V(1,:);
	Hi_D	=	V(2,:);
	Lo_R	=	V(3,:);
	Hi_R	=	V(4,:);
	...
	if(plot_bool)
		figure; subplot(3,1,1); plot(mag2db((X))); title('Clean signal with scaled wavelets added')
				subplot(3,1,2); plot(mag2db((XN))); title('Noisey signal with scaled wavelets added')
				subplot(3,1,3); plot(mag2db((recovered))); title(['Denoised Reconstructed Signal using rigrsure with ', wname])
